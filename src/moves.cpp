...
Rcpp::List cpp_move_tau(Rcpp::List param, Rcpp::List data, Rcpp::List config,
  ...
  Rcpp::NumericVector move_tau = config["move_tau"]; // these are just pointers
  ...
  Rcpp::List p_trans_list = Rcpp::as<Rcpp::List>(data["p_trans"]);
  Rcpp::List p_trans_int_list = Rcpp::as<Rcpp::List>(data["p_trans_int"]);
  ...
  for (size_t i = 0; i < tau.size(); i++) {
    ...
    if(move_tau[i] && unif_rand() < prop_tau_move) {
      ...
      new_tau[i] += R::rnorm(0.0, sd_tau[i]); // new proposed value
	...
	return param;
      ...
      Rcpp::NumericMatrix p_trans_int = Rcpp::as<Rcpp::NumericMatrix>(p_trans_int_list[i]);
      ...
      new_trans_mat_1[i] = get_transition_mat(p_trans,
					      p_trans_int,
					      eps[i + n_undated],
					      tau[i],
					      1);
...
Rcpp::List cpp_move_eps(Rcpp::List param, Rcpp::List data, Rcpp::List config,
  ...
  for (size_t i = 0; i < eps.size(); i++) {
    ...
    if((move_eps[i] && ind < 0) || (move_eps[i] && ind >= 0 && unif_rand() < prop_eps_move)) {
	...
	Rcpp::NumericMatrix p_trans_int = Rcpp::as<Rcpp::NumericMatrix>(p_trans_list[ind]);
	...
	new_trans_mat_1[ind] = get_transition_mat(p_trans,
						  p_trans_int,
						  new_eps[i],
						  1,
						  1);
...
Rcpp::List cpp_move_t_inf(Rcpp::List param, Rcpp::List data,
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    old_loc_loglike = cpp_ll_timing(data, param, i+1, list_custom_ll) +
      cpp_ll_timeline(data, param, i+1, list_functions["timeline"]); // term for case 'i' with offset
    ...
    // we only add ll_timeline for case i, not the local cases (these are not
    // affected by a change in infection time)
...
Rcpp::List cpp_move_model(Rcpp::List param, Rcpp::List data, Rcpp::List config,
  ...
  Rcpp::IntegerVector kappa = param["kappa"]; // pointer to param$kappa
  Rcpp::NumericMatrix place = param["place"]; // pointer to param$kappa
  ...
  for (size_t i = 0; i < N; i++) {
	...
	if(t_inf[i] - t_inf[new_alpha[i] - 1] < 2) {
	  ...
	  // Propose a new place randomly from all possible places. We might
	  // have to change this to non-random proposals (i.e. propose the place
	  // of the infector, and then adjust for this in the likelihood) to
	  // improve mixing if we have a large number of possible places. We
	  // also propose place = 0 (i.e. they are not in a hospital on that
	  // day) - if this is not wanted, make sure the transition
	  // probabilities from and to place = 0 are set to zero.
	  for(size_t j = 0; j < place.nrow(); j++) {
	    Rcpp::IntegerVector place_seq = Rcpp::seq(0, N_place[j]);
	    new_place(j, i) = Rcpp::as<int>(Rcpp::sample(place_seq, 1, true));
	  }

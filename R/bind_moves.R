...
bind_moves <- function(moves = custom_moves(), config, data,
  ...
  ## remove move$R if disabled
  if (!any(config$move_R)) {
    out$R <- NULL
  } else {
    out$R <- bind_to_function(out$R,
                              data = data,
                              config = config,
                              custom_ll = likelihoods$offspring,
                              custom_prior = priors$R
                              )
  }


  ## remove move$k if disabled
  if (!any(config$move_k)) {
    out$k <- NULL
  } else {
    out$k <- bind_to_function(out$k,
                              data = data,
                              config = config,
                              custom_ll = likelihoods$offspring,
                              custom_prior = priors$k
                              )
  }

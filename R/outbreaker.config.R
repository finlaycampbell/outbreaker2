...
#' 
#' \item{init.xi}{initial value for the false contact rate}
#'
...
#' should be estimated ('moved' in the MCMC), or not, all defaulting to TRUE.}
#' 
#' \item{move.xi}{a logical indicating whether the false contact rate
#' should be estimated ('moved' in the MCMC), or not, all defaulting to TRUE.}
#'
...
#' 
#' \item{sd.xi}{the standard deviation for the Normal proposal for the false contact rate}
#'
...
#'
#' \item{prior.xi}{a numeric vector of length 2 indicating the upper and lower bounds of the
#' uniform prior for the false contact rate 'xi'}
#'
#' }
#'
...
outbreaker.config <- function(..., data=NULL, config=NULL) {
  ...
  defaults <- list(init.tree=c("seqTrack","star","random"),
                   ...
                   move.mu=TRUE, move.kappa=TRUE, move.pi=TRUE, move.eps=TRUE, move.xi=TRUE,
                   n.iter=1e4, sample.every=200, sd.mu=0.0001, sd.pi=0.01, sd.eps=0.01, sd.xi=0.01,
                   prop.alpha.move=1/4,
                   prop.t.inf.move=0.2,
                   batch.size=1e6,
                   paranoid=FALSE,
  ...
  ## check init.xi
  if (!is.numeric(config$init.xi)) {
    stop("init.xi is not a numeric value")
  }
  if (config$init.xi < 0) {
    stop("init.xi is negative")
  }
  if (config$init.xi > 1) {
    stop("init.xi is greater than 1")
  }
  if (!is.finite(config$init.xi)) {
    stop("init.xi is infinite or NA")
  }
  ...
  ## check move.xi
  if (!is.logical(config$move.xi)) {
    stop("move.xi is not a logical")
  }
  if (is.na(config$move.xi)) {
    stop("move.xi is NA")
  }
  ...
  ## check sd.xi
  if (!is.numeric(config$sd.xi)) {
    stop("sd.xi is not a numeric value")
  }
  if (config$sd.xi < 1e-10) {
    stop("sd.xi is close to zero or negative")
  }
  if (!is.finite(config$sd.xi)) {
    stop("sd.xi is infinite or NA")
  }
  ...
  ## check prior value for xi
  if (!all(is.numeric(config$prior.xi))) {
    stop("prior.xi has non-numeric values")
  }
  if (any(config$prior.xi < 0)) {
    stop("prior.xi has negative values")
  }
  if (length(config$prior.xi)!=2L) {
    stop("prior.xi should be a vector of length 2")
  }
  if (!all(is.finite(config$prior.xi))) {
    stop("prior.xi is has values which are infinite or NA")
  }

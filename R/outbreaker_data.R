...
outbreaker_data <- function(..., data = list(...)) {
  ...
  ## CHECK T_END
  if (!is.null(data$t_end)) {
    if (!is.numeric(data$t_end)) {
      stop("t_end is not a number")
    }
    if (data$t_end < 0 | data$t_end%%1 != 0) {
      stop("t_end should be a positive integer")
    }
    if (!is.finite(data$t_end)) {
  ...
  data$est_f_dens <- .get_est_f(data$dates, data$f_dens, data$t_end)
